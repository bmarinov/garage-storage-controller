...
test: manifests generate fmt vet setup-envtest ## Run tests.
	KUBEBUILDER_ASSETS="$(shell "$(ENVTEST)" use $(ENVTEST_K8S_VERSION) --bin-dir "$(LOCALBIN)" -p path)" ./scripts/test.sh
...
test-e2e: setup-test-e2e manifests generate fmt vet ## Run the e2e tests. Expected an isolated environment using Kind.
	KIND=$(KIND) KIND_CLUSTER=$(KIND_CLUSTER) RUN_E2E=true go test ./test/e2e/ -v -ginkgo.v
...
.PHONY: docker-buildx
docker-buildx: ## Build and push docker image for the manager for cross-platform support (set PUSH=true to push)
	...
	- $(CONTAINER_TOOL) buildx build $(if $(PUSH),--push) --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
